...
void initPlayer() {
    ...
    player.timestep = 2;
    player.powerup = 0;
    player.color = ((19&31) | (11&31) << 5 | (7&31) << 10);
...
void initBee() {
    ...
    bee.right = 0;
    bee.down = 0;
    bee.color = ((30&31) | (30&31) << 5 | (21&31) << 10);
}
...
void initFlowers() {
    for (int i = 0; i < 25; i++) {
        ...
        switch (colorPicker) {
            case 0:
                flowers[i].color = ((30&31) | (30&31) << 5 | (21&31) << 10);
                break;
            case 1:
                flowers[i].color = ((18&31) | (19&31) << 5 | (27&31) << 10);
                break;
            case 2:
                flowers[i].color = ((31&31) | (13&31) << 5 | (21&31) << 10);
                break;
            case 3:
                flowers[i].color = ((17&31) | (27&31) << 5 | (31&31) << 10);
                break;
            case 4:
                flowers[i].color = ((31&31) | (31&31) << 5 | (31&31) << 10);
                break;
        }
    }
...
void updateFlowers(FLOWER* f) {
    if (f->active) {
        if (collision(bee.x-2,bee.y,bee.width,bee.height,f->x,f->y,4,9)) {
            ...
            *(volatile u16*)0x0400006C = NOTE_G6 | (1<<15) | (1<<14);
            bee.color = f->color;
        }
        if (collision(player.x,player.y-4,player.width,player.height,f->x,f->y,4,9)) {
            ...
            playerScore++;
            *(volatile u16*)0x04000068 = (((5) & 15) << 12) | (((7) & 7) << 8);
            *(volatile u16*)0x0400006C = NOTE_E6 | (1<<15) | (1<<14);
        }
    }
}
...
void updatePlayer() {
    ...
    if (framecount % player.timestep == 0) {
        ...
        if ((~(buttons) & ((1<<4))) && (player.x < 232)) {
            player.x += 1;
        }
        if ((~(buttons) & ((1<<6))) && (player.y > 5)) {
            player.y -= 1;
        }
        if ((~(buttons) & ((1<<7))) && (player.y < 156)) {
            player.y += 1;
        }
    }
    ...
    if (collision(player.x,player.y-4,player.width,player.height,bee.x-2,bee.y,bee.width,bee.height)) {
        ...
        slowdown = 1;
        player.timestep = 3;
        player.color = ((17&31) | (27&31) << 5 | (31&31) << 10);
    }
    ...
    if (slowdown == 1) {
        *(volatile u16*)0x04000068 = (((5) & 15) << 12) | (((7) & 7) << 8);
        if (count >= 0 && count < 15) {
            *(volatile u16*)0x0400006C = NOTE_G3 | (1<<15) | (1<<14);
            count++;
        } else if (count >= 15 && count < 30) {
            *(volatile u16*)0x0400006C = NOTE_F3 | (1<<15) | (1<<14);
            count++;
        } else if (count >= 30 && count < 45) {
            *(volatile u16*)0x0400006C = NOTE_E3 | (1<<15) | (1<<14);
            count++;
        } else {
            *(volatile u16*)0x0400006C = NOTE_E3 | (1<<15) | (1<<14);
            count = 0;
...
void drawFlowers(FLOWER* f) {
    if (f->active) {
        ...
        drawRect(f->x, f->y, f->width, f->height, ((21&31) | (25&31) << 5 | (20&31) << 10));
        f->erased = 1;
    }
}
...
void drawPlayer() {
    drawBasketCover(player.oldx, player.oldy, ((21&31) | (25&31) << 5 | (20&31) << 10));
    drawBasket(player.x,player.y, player.color);
}


void drawGameBee() {
    drawBeeCover(bee.oldx,bee.oldy,((21&31) | (25&31) << 5 | (20&31) << 10));
